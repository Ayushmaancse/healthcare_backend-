...
6. **WSGI vs ASGI Gateway Flexibility**: The project natively ships with both `wsgi.py` and `asgi.py` entry points. The application is currently served via WSGI for optimized synchronous REST API traffic. However, maintaining the ASGI interface ensures the architecture is "future-proofed" for instant drop-in support of real-time features (like WebSockets) without requiring a core framework restructure.
